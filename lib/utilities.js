...
// Keys that must never be used when walking/creating nested objects, because
// they can be leveraged to pollute the global Object prototype.
const FORBIDDEN_NESTED_KEYS = new Set(['__proto__', 'prototype', 'constructor']);
...
const isSafeFromPollution = (base, key) => {
  // Reject well-known pollution vectors regardless of the base object type.
  if (FORBIDDEN_NESTED_KEYS.has(key)) return false;
...
/**
 * Sanitize file name to prevent path traversal attacks. Strips directory
 * components and traversal sequences so the result is always a bare file
 * name safe to combine with a destination directory.
 * @param {string} fileName - file name to sanitize.
 * @returns {string}
 */
const sanitizeFileName = (fileName) => {
  if (!fileName || typeof fileName !== 'string') return '';
  // Remove NUL bytes which can be used to truncate paths in some contexts.
  let sanitized = fileName.replace(/\0/g, '');
  // Replace backslashes (Windows path separators) with forward slashes so that
  // subsequent stripping catches them consistently regardless of platform.
  sanitized = sanitized.replace(/\\/g, '/');
  // Strip any directory components so only the base name remains.
  sanitized = sanitized.substring(sanitized.lastIndexOf('/') + 1);
  // Use path.basename as an additional safety net (handles remaining separators
  // on the current platform).
  sanitized = path.basename(sanitized);
  // Reject names that resolve to traversal identifiers or are otherwise empty.
  if (!sanitized || sanitized === '.' || sanitized === '..') return '';
  // Defensive: strip any leading dots-and-slashes patterns (e.g. "..", "../").
  // After the basename step this should already be gone, but guard against
  // pathological input just in case.
...
const parseFileName = (opts, fileName) => {
  ...
  // Always sanitize the file name to strip any path separators or traversal
  // components before it can be used to build a destination path. This is
  // done regardless of the safeFileNames option to prevent path traversal
  // (CVE-class: arbitrary file write via crafted Content-Disposition filename).
  parsedName = sanitizeFileName(parsedName);
  // If sanitization removed everything, fall back to a generated temp name.
  if (!parsedName) return getTempFilename();
