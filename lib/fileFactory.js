 ...
 * Strips any path components, rejects NUL bytes, and falls back to an empty
 * string for non-string or otherwise unusable values.
 * @param {string} name
 * @returns {string}
 */
const safeFilename = (name) => {
  if (typeof name !== 'string') return '';
  // Reject NUL bytes outright — they can be used to truncate paths in
  // downstream native calls.
  if (name.indexOf('\0') !== -1) return '';
  // Normalize backslashes to forward slashes before calling path.basename
  // to ensure robust directory traversal sanitization across all platforms.
  const base = path.basename(name.replace(/\\/g, '/'));
  // Explicitly discard traversal-only names.
  if (base === '..' || base === '.') return '';
  return base;
};
